...
const Sidebar = () => {
    ...
    const { user, isUserLoading, axios, fetchUser } = useAppContext()
    const location = useLocation();
    const [image, setImage] = React.useState('');
    const serverBaseUrl = (import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api').replace(/\/api\/?$/, '');

    const resolvedUserImage = user?.image
        ? (user.image.startsWith('http') ? user.image : `${serverBaseUrl}${user.image}`)
        : '';

    const cacheBustedUserImage = resolvedUserImage
        ? `${resolvedUserImage}${resolvedUserImage.includes('?') ? '&' : '?'}t=${encodeURIComponent(user?.updatedAt || '')}`
        : '';
