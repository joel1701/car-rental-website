...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const {data} = await axios.get('/owner/dashboard-data') 
      if(data.success){
            ...
            {data.recentBookings.map((booking, index)=>(
              <div key={index} className='mt-4 flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                 ...
                 <div>
                  <p>{booking.car?.brand || "Deleted"} {booking.car?.model || "Car"}</p>
                  <p className='text-sm tex-gray-500'>{booking.createdAt.split('T')[0]}</p>
                 </div>
                </div>
